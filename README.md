...
```bash
solidcore-poc-events$ sam build
```
...
```bash
solidcore-poc-events$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
solidcore-poc-events$ sam local start-api
solidcore-poc-events$ curl http://localhost:3000/
```
...
```bash
solidcore-poc-events$ sam logs -n HelloWorldFunction --stack-name solidcore-poc-events --tail
```
...
```bash
solidcore-poc-events$ cd hello-world
...
```bash
sam delete --stack-name solidcore-poc-events
```
